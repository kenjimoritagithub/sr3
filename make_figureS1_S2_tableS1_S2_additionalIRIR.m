% make_figureS1_S2_tableS1_S2_additionalIRIR

% run simulations by using "broadsim" and save data for Figures S1 and S2 and Tables S1 and S2
...
% make Figures S1 and S2 and Table S1
for k_model = 1:3
    for k_g = 1:length(g_set)
        for k_b = 1:length(b_set)
...
% make the color bar for Figures S1 and S2
F = figure;
A = axes;
hold on;
P = colorbar;
set(P,'YTick',[1 65],'YTickLabel',[0 Pfmax_all],'FontSize',40);
print(F,'-depsc','FigureS1-S2-colorbar');
