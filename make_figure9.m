...
tmp_letters{1} = 'B'; tmp_letters{2} = 'D1'; tmp_letters{3} = 'D2'; tmp_letters{4} = 'D3';
for k1 = 1:length(c_set)
    heatmap_performance(mean(totalRset{k1},3),length(a_posiprop_set),[tmp_max tmp_min],['Figure9' tmp_letters{k1}]);
end
...
save data_SRIRex_Figure9C Out_set
%
for k1 = 1:2
    F = figure;
    A = axes;
    ...
    print(F,'-depsc',['Figure9Ca' num2str(k1)]);
end
%
...
for k1 = 1:2
    ...
    print(F,'-depsc',['Figure9C' tmp_letters(k1)]);
end
